...
Evaluate the trained Baseline (ResNetRegressor) model on the test set.

This script evaluates the single-view baseline model and generates:
...
Usage:
    ...
    Custom parameters:
        python Model/Evaluating/evaluate_baseline.py --split test --batch_size 32

Outputs:
    - Results/plots/eval_baseline_{split}_scatter.png - Scatter plot
    - Results/plots/eval_baseline_{split}_confusion_matrix.png - Confusion matrix
    - Results/predictions/predictions_baseline_{split}.csv - All predictions
    - Console output with MAE and RMSE metrics

Requirements:
    - Model must be trained first: python Model/Training/train_baseline.py
    - Checkpoint must exist at: Model/checkpoints/best_baseline.pth
"""
...
from Model.dataset import ChickenAgeDataset, get_default_transforms  # noqa: E402
...
def main():
    ...
    print("=" * 80)
    print("EVALUATING BASELINE MODEL (ResNetRegressor)")
    print("=" * 80)
    print(f"\nEvaluating on {args.split} set...")

    # Paths
    csv_path = _project_root / "Labels" / f"{args.split}.csv"
    checkpoint_path = _project_root / "Model" / "checkpoints" / "best_baseline.pth"
    ...
    if not checkpoint_path.exists():
        ...
        print("  python Model/Training/train_baseline.py")
        return 1

    # Load data
    dataset = ChickenAgeDataset(csv_path, root_dir, transforms=get_default_transforms(train=False))
    ...
    csv_output = output_dir / f"predictions_baseline_{args.split}.csv"
    ...
    scatter_path = plot_dir / f"eval_baseline_{args.split}_scatter.png"

    plt.figure(figsize=(7, 7))
    plt.scatter(gts_arr, preds_arr, alpha=0.6, s=30, color='#1f77b4')
    ...
    plt.title(f"Baseline Model - {args.split.capitalize()} Set\nMAE: {mae_val:.3f}, RMSE: {rmse_val:.3f}", fontsize=13)
    ...
    plt.title(f"Baseline Model - Confusion Matrix ({args.split.capitalize()} Set)\nMAE: {mae_val:.3f}", fontsize=13)
    plt.tight_layout()

    cm_path = plot_dir / f"eval_baseline_{args.split}_confusion_matrix.png"
