"""evaluate_latefusion.py
Evaluate the trained Late Fusion model on the test set.
...
Usage:
    ...
    Evaluate on validation set:
        python Model/Evaluating/evaluate_latefusion.py --split val
...
Requirements:
    - Model must be trained first: python Model/Training/train_latefusion.py
    - Checkpoint must exist at: Model/checkpoints/best_late_fusion.pth
...
from Model.model import LateFusionRegressor  # noqa: E402
...
def main():
    ...
    print("=" * 80)
    print("EVALUATING LATE FUSION MODEL (Prediction Averaging)")
    print("=" * 80)
    print(f"\nEvaluating on {args.split} set...")

    # Paths
    csv_path = _project_root / "Labels" / f"{args.split}.csv"
    checkpoint_path = _project_root / "Model" / "checkpoints" / "best_late_fusion.pth"
    ...
    if not checkpoint_path.exists():
        ...
        print("  python Model/Training/train_latefusion.py")
        return 1
    ...
    scatter_path = plot_dir / f"eval_latefusion_{args.split}_scatter.png"

    plt.figure(figsize=(7, 7))
    plt.scatter(gts_arr, preds_arr, alpha=0.6, s=30, color='#ff7f0e')
    ...
    plt.title(f"Late Fusion Model - {args.split.capitalize()} Set\nMAE: {mae_val:.3f}, RMSE: {rmse_val:.3f}", fontsize=13)
